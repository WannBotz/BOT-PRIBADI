...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
    ...
    let regc = /(aku sayang kamu|aku syg kmu|aku syg kamu|aku sayang kamuhh)/i
...
let lopiuh = lopu[Math.floor(Math.random() * cinmu.length)]
    if (isLoPiu && !m.fromMe) {
    conn.sendMessage(m.chat, {
        react: {
          text: `${lopiuh}`,
   ...
   setTimeout(() => {
        conn.reply(m.chat, `lopiuuu juga sayang ${lopiuh}`, m)
    }, 1000)
    }
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
    ...
    let regl = /(I love you|love you|love you sayang|i love u)/i
...
let loveyou = lovou[Math.floor(Math.random() * lovou.length)]
    ...
    let helloaine = fs.readFileSync('./mp3/loveyousyg.opus') 
    conn.sendMessage(m.chat, {
        react: {
          text: `${loveyou}`,
          key: m.key,
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
    ...
    let ass = /(assalamualaikum|Assallamualaikum|assallamualaikum|assalamu\'alaikum|as\'salamualaikum)/i
